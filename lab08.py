...
def tryToConnect():
    ...
    msg = g_app.ipPort.get()

    (addr, portStr) = tuple(filter(None, msg.split(':')))
    portNum = int(portStr)

    

    if g_sock.connect((addr, portNum)):
        printToMessages("Error, server not found")
...
def sendMessage(master):
    ...
    try: 
        g_sock.sendall(bytearray(msg, "ASCII"))
